...
describe('Message Command Handlers', () => {
  ...
  describe('sendMessageService', () => {
    it('should send a message and the message returned should have a id', async () => {
      ...
      const messageToSendCopy: ISendMessageRequest = JSON.parse(
        JSON.stringify(messageToSend)
      );
      const result: ISendMessageResponse = {
        message: messageToSendCopy.conversation.messages![0],
      };
      const send = await commandBus.execute(
        new SendMessageCommand(messageToSend)
      );
      console.log(send);
  ...
  describe('CreateConversation', () => {
    it('Create A new Conversation between two users', async () => {
      ...
      const newConversation: ICreateConversationRequest = {
      ...
      const compare: ICreateConversationRequest = JSON.parse(
        JSON.stringify(newConversation)
      );
      const sent = await commandBus.execute(
        new CreateConversationCommand(newConversation)
      );
