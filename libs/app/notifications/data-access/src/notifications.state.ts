...
export class NotificationsState {
  ...
  subscribeToNotifications(ctx: StateContext<NotificationsStateModel>) {
    ...
    return this.notificationsApi
      ...
      .pipe(
        tap((notificationBox: INotificationBox) =>
          ctx.dispatch(new SetNotifications(notificationBox))
        )
      );
  }
    ...
    try {
      ...
      if (currNotificationsBox != undefined) {
        for (let i = 0; i < currNotificationsBox?.inbox.length; i++) {
          if (
            currNotificationsBox?.inbox[i].notificationID ==
      ...
      const responseRef = await this.notificationsApi.markNotificationsAsRead(
        request
      );
