...
import { ILikedAndDisliked } from '@mp/app/feed/util';
...
export class FeedApi {
  ...
  post$(id: string) {
    const docRef = doc(this.firestore, `post/${id}`).withConverter<IPost>({
  ...
  async LikePost(request: ILikePostRequest) {
    console.log(
      'feed.api LikePost working: ' +
        request.post.postId +
        ' ' +
        request.post.userId
    );
    return await httpsCallable<ILikePostResponse, ILikePostRequest>(
      this.functions,
      'likePost'
    )(request);
  }
  ...
  async DislikePost(request: IDislikePostRequest) {
    ...
    return await httpsCallable<IDislikePostResponse, IDislikePostRequest>(
      this.functions,
      'dislikePost'
    )(request);
  }
  ...
  async getProfileLikedAndDisliked(userID: string): Promise<ILikedAndDisliked> {
    ...
    const filteredLikedRefs = existingLikedRefs.filter(
      (ref) => ref !== null
    ) as DocumentReference[];
    const filteredDislikedRefs = existingDislikedRefs.filter(
      (ref) => ref !== null
    ) as DocumentReference[];
  ...
  async GetHomeFeed$(
    ...
    const profileUserIds = profiles.map((profile) => {
      return profile.userId;
    });
    if (profileUserIds.length > 0) {
      const filter = query(allPosts, where('userId', 'in', profileUserIds));
  ...
  async GetDiscoveryFeed(
    ...
    const allUsers = collection(this.firestore, 'profiles'); // find profiles that fit criteria
    try {
      const challengesAndDepartmentFilter = query(
        allUsers,
        where('challenges', 'not-in', feed.user.challenges)
      );
      const excludeCurrent = query(
        challengesAndDepartmentFilter,
        where('userId', '!=', feed.user.userId)
      );
      const querySnapshot = await getDocs(excludeCurrent);
      const profiles: IProfile[] = querySnapshot.docs.map((doc) => {
  ...
  async SendComment(request: IUpdateCommentsRequest) {
    ...
    return await httpsCallable<IUpdateCommentsRequest, IUpdateCommentsResponse>(
      this.functions,
      'updateComments'
    )(request);
  }
